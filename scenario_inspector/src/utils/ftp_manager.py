...
class FTPManager:
    ...
    def test_connection(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.debug(f"Testing FTP connection to {host}:{port}")
            ...
            self.logger.debug(f"Passive mode set to: {passive_mode}")
            ...
            files = ftp.nlst()
            self.logger.debug(f"Successfully connected. Files in root: {len(files)} items")
            ...
            if base_path and base_path != '/':
                try:
                    ftp.cwd(base_path)
                    self.logger.debug(f"Successfully accessed base path: {base_path}")
                except ftplib.error_perm as e:
                    self.logger.debug(f"Base path {base_path} not accessible: {e}")
                    # Try to create the base path
                    try:
                        ftp.mkd(base_path)
                        ftp.cwd(base_path)
                        self.logger.debug(f"Created and accessed base path: {base_path}")
    ...
    def _create_directory_if_not_exists(self, directory: str) -> bool:
        ...
        try:
            ...
            try:
                ...
                self.ftp_connection.cwd(directory)
                self.logger.debug(f"Created FTP directory: {directory}")
                return True
            except ftplib.error_perm as e:
                self.logger.error(f"Failed to create FTP directory {directory}: {e}")
                return False
    ...
    def upload_inspection_folder(self, local_folder_path: str, inspection_date: datetime = None) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.debug(f"Uploading {local_folder_path} to FTP as {date_folder}/{inspection_folder_name}")
            ...
            for root, dirs, files in os.walk(local_folder_path):
                ...
                for file in files:
                    ...
                    try:
                        ...
                        self.logger.debug(f"Uploaded: {ftp_file_path}")
                    except Exception as e:
                        failed_files.append(f"{ftp_file_path}: {str(e)}")
                        self.logger.error(f"Failed to upload {ftp_file_path}: {e}")
